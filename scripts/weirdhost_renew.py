...
def _is_cf_challenge(sb):
    """通过 DOM/标题判断当前是否仍处于 Cloudflare 全页验证（含点击后转圈确认阶段）"""
    try:
        for sel in ("iframe[src*='challenges.cloudflare.com']",
                    "#challenge-running", "#challenge-stage", "#challenge-form",
                    "#cf-chl-widget", "#cf-please-wait", "#cf-spinner-please-wait"):
            if sb.is_element_present(sel):
                return True
        title = (sb.get_title() or "").strip().lower()
        keywords = ("just a moment", "please wait", "attention required",
                    "잠시만", "확인하는 중", "보안 확인", "사람인지")
...
def _wait_cf_pass(sb, timeout=90):
    """等待 Cloudflare 全页验证自行完成——验证通过后页面会自动跳转，
    千万不要在转圈期间主动导航，否则 clearance 永远拿不到（验证死循环）"""
    start = time.time()
    while time.time() - start < timeout:
...
def process_single_account(sb, account, account_index):
    ...
    sb.add_cookie({"name": cookie_name, "value": cookie_value, "domain": DOMAIN, "path": "/"})
    # GitHub 机房 IP 信誉差，注入 Cookie 后的重新加载可能再次触发
    # Cloudflare 全页人机验证；且验证通过后页面还会停留数秒，必须等它结束
    for cf_attempt in range(1, 3):
        sb.uc_open_with_reconnect(f"https://{DOMAIN}/", reconnect_time=5)
        handle_turnstile(sb)
        if _wait_cf_pass(sb, timeout=45):
            break
        print(f"[WARN]   Cloudflare 验证页未完成（第 {cf_attempt}/2 次），重新加载...")
    time.sleep(3)

    if not is_logged_in(sb):
        for attempt in range(1, 3):
            print(f"[WARN]   未检测到登录状态，尝试刷新（{attempt}/2）...")
...
# 这些状态与网络/节点无关，切换节点重试没有意义
_NO_RETRY_STATUSES = {"success", "cooldown", "skipped", "no_server", "cookie_invalid"}

_PROXY_NODE_INDEX = 0


def _proxy_node_count():
    raw = os.environ.get("PROXY_URL", "").strip()
    if not raw:
        return 0
    return len([p for p in re.split(r"[\s;]+", raw) if "://" in p])


def _restart_proxy():
    """重启 sing-box；PROXY_URL 配置了多个节点时切换到下一个节点。"""
    if not os.path.exists("sing-box"):
        print("[WARN] 当前环境没有 sing-box 可执行文件，跳过节点切换")
        return
    global _PROXY_NODE_INDEX
    node_total = _proxy_node_count()
    if node_total > 1:
        _PROXY_NODE_INDEX = (_PROXY_NODE_INDEX % node_total) + 1
        print(f"[INFO] 切换代理节点 -> #{_PROXY_NODE_INDEX}/{node_total}")
        env = dict(os.environ)
        env["PROXY_INDEX"] = str(_PROXY_NODE_INDEX)
        subprocess.run([sys.executable, "proxy_handler.py"], env=env)
    else:
        print("[INFO] 重启 sing-box ...")
...
def add_server_time():
    ...
    proxy_server = os.environ.get("PROXY_SERVER", "").strip() or "http://127.0.0.1:8080"
    try:
    ...
    def _error_result(account, idx, err):
        return {
            "remark": account.get("remark", f"账号{idx + 1}"),
            "cookie_env": account.get("cookie_env", ""),
            "email": "Unknown",
            "status": "error",
            "message": f"浏览器异常: {repr(err)}",
            "servers": [],
            "cookie_updated": False,
        }
    ...
    try:
        if is_proxy:
            print(f"\n[INFO] 🔗 代理模式: {proxy_server}")
            if _proxy_node_count() > 1:
                print(f"[INFO] 节点池: {_proxy_node_count()} 个，失败自动轮换（每账号最多 {max_attempts} 次尝试）")
            for i, account in enumerate(accounts):
                result = None
                for attempt in range(1, max_attempts + 1):
                    print(f"\n[INFO] ===== 账号 [{i + 1}] 第 {attempt}/{max_attempts} 次尝试 =====")
                    if attempt > 1:
                        _restart_proxy()
                    try:
                        ...
                        print(f"\n[ERROR] 浏览器异常: {repr(e)}")
                        traceback.print_exc()
                        result = _error_result(account, i, e)
                    # 结果已有明确结论（success/cooldown/skipped 等）时停止重试，
                    # 只有网络类失败（error/timeout/unknown）才换节点重试
                    if result and result.get("status") in _NO_RETRY_STATUSES:
                        break
                if result is None:
                    result = _error_result(account, i, RuntimeError("no result"))
